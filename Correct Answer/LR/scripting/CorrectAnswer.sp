...
public Action PlayerSay(Event hEvent, const char[] sEvent, bool bdb)
{
	...
	if(0 < client <= MaxClients && IsClientInGame(client) && bCanSend)
	{
		...
		{
			LR_ChangeClientValue(client, REW);
			CGOPrintToChatAll("Игрок {OLIVE}%N{DEFAULT} правильно ввел слово \"{GREEN}%s{DEFAULT}\" и получил {RED}%d{DEFAULT} опыта", client, sWord, REW);
			bCanSend = false;
		}	
	}
...
public Action Rotation(Handle hTimer)
{
	...
	TrimString(sWord);
	CGOPrintToChatAll("Напиши слово \"{GREEN}%s{DEFAULT}\" и получи {RED}%d{DEFAULT} опыта", sWord, REW);
	bCanSend = true;
